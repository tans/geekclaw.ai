...
export default async function ProductTagOpsPanel(props: BeforeListServerProps) {
  const [tagsResult, productsResult] = await Promise.all([
    props.payload
      .find({
    ...
    <section style={panelStyle}>
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
        <div>
          ...
          <h2 style={{ margin: '10px 0 0', fontSize: 24 }}>商品标签运营</h2>
          <p style={descStyle}>标签适合交付形态、硬件能力、库存策略和运营标签。这里提供标签使用概况和快捷筛选入口。</p>
        </div>
        <a href="/admin/collections/product-tags" style={buttonPrimary}>
          管理商品标签
        </a>
      </div>
      ...
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
        ...
        {tagSummaries.map((tag) => (
          <a key={tag?.id} href={`/admin/collections/products?where[tags][contains]=${encodeURIComponent(String(tag?.id || 0))}`} style={chipSecondary}>
